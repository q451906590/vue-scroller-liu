...
import XScroller from 'xscroll/build/cmd/xscroll.js'
...
export default {
  ...
  methods: {
    ...
    donePulldown () {
      ...
      this.currentValue.pulldownSattus = 'default'
    },
    donePullup () {
      this.pullup.complete()
      this.reset()
      this.currentValue.pullupSattus = 'default'
    },
    disablePullup () {
      this.pullup.stop()
  ...
  watch: {
    ...
    value: {
      handler: function (val) {
        ...
        if (val.pullupStatus === 'disabled' && this.currentValue.pulldownStatus !== 'disabled') {
          this.disablePullup()
        }
        if (val.pullupStatus === 'enabled' && this.currentValue.pulldownStatus === 'disabled') {
          this.enablePullup()
        }
      },
      deep: true
  ...
  mounted () {
    ...
    this.$nextTick(() => {
      ...
      if (this.usePullup) {
        ...
        let config = objectAssign(pulldownDefaultConfig(), this.pullupConfig)
        if (container) {
          config.container = container[0].elm
        }
